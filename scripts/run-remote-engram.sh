...
PROFILE="$ROOT/profiles/axon.remote-engram.jsonc"
...
if [[ -z "${ENGRAM_WORKER_URL:-}" ]]; then
  echo "ENGRAM_WORKER_URL is required (remote Engram host, e.g. http://192.168.1.5:37779)." >&2
  exit 1
fi
